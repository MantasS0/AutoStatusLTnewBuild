...
public class Data_Repository implements AsyncResult {
    ...
    private MutableLiveData<List<Room_Data>> vehicleLiters = new MutableLiveData<>();
    private LiveData<List<Room_Data>> vehiclePrice;
    private LiveData<List<Room_Data>> vehicleDistance;

    public MutableLiveData<List<Room_Data>> vehicleData = new MutableLiveData<>();
...
//    public void getAllData(Room_Data room_data) {
//        new queryAsyncTask(mDataDAO).;
//    }

    public void insertEntry(Room_Data newRoom_data) {
        new queryAsyncTask.insertAsyncTask(mDataDAO).execute(newRoom_data);
    }
    ...
    public void findVehicleData(Integer vehicleCounter) {
        ...
        vehicleData = searchResults;
    }

    public void findVehicleLiters(Integer vehicleCounter) {
        queryAsyncTask task = new queryAsyncTask(mDataDAO);
        task.delegate = this;
        task.execute(vehicleCounter);
    }
    ...
    public MutableLiveData<List<Room_Data>> getVehicleLiters() {
        return vehicleLiters;
    }

    public LiveData<List<Room_Data>> getVehiclePrice() {
        return vehiclePrice;
    }

    public LiveData<List<Room_Data>> getVehicleDistance() {
...
//    }
    ...
    private static class queryAsyncTask extends AsyncTask<Integer, Void, List<Room_Data>> {
        ...
        queryAsyncTask(Room_Data_DataAccessObject dao) {
            asyncTaskDao = dao;
        }
...
//        private static class getAllDataAsyncTask extends AsyncTask<Void, Void, List<Room_Data>> {
//            private Room_Data_DataAccessObject asyncTaskDao;
//
//            getAllDataAsyncTask(Room_Data_DataAccessObject dao) {
//                asyncTaskDao = dao;
//            }
//
//            @Override
